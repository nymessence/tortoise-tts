...
def run_generation_for_spawn(index, flags):
    ...
    """
    # Import necessary torch_xla modules here, inside the function, to ensure they are loaded
    # in each child process correctly.
    ...
    # Dynamically calculate the step and start index based on available TPU cores
...
# ==================================
# Pre-processing, audio processing, and main logic
# ==================================
...
def run_generation(args):
    ...
    if args.hardware == 'tpu':
        # xm is already loaded in the run_generation_for_spawn function
        if xm is None:
            import torch_xla.core.xla_model as xla_model
            xm = xla_model
...
def post_process_audio(out_dir, preprocessed_script_path):
    ...
    """
    # Import modules here to avoid issues with multiprocessing
    from pydub import AudioSegment
    import numpy as np
    from scipy.io.wavfile import write
...
def main():
    ...
    if 'TPU_PROCESSES_COUNT' in os.environ and 'TPU_NAME' in os.environ:
        try:
            logging.info("[INFO] ✅ TPU detected. Starting generation with xmp.spawn.")
            import torch_xla.distributed.xla_multiprocessing as xmp
            # Use nprocs=None to let the library auto-detect and use all cores
            xmp.spawn(run_generation_for_spawn, args=(flags,), nprocs=None, start_method='fork')
        except Exception as e:
            logging.warning(f"[INFO] ⚠️ TPU multiprocessing failed ({e}). Falling back to single-process generation.")
            # Fallback to GPU or CPU if TPU setup fails
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            flags['hardware'] = device
            run_generation_local(flags)
    else:
        # Fallback if not a TPU environment
        logging.info("[INFO] ⚠️ TPU not detected. Falling back to single-process generation.")
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        flags['hardware'] = device
        run_generation_local(flags)

    # All post-processing logic (stitching, etc.)
    post_process_audio(OUT_DIR, PREPROCESSED_SCRIPT_PATH)
